...
Hay virtualmente infinitas maneras de diseñar software para que realice una determinada tarea.
Y otras infinitas maneras de implementarlo. La herramienta desarrollada en esta tesis, denominada [FeenoX](https://www.seamplex.com/feenox/), fue diseñada siguiendo un patrón frecuente en la industria de software. El "cliente" define un documento titulado [_Sofware Design Requirements_]{lang=en-US} y el "proveedor" indica cómo cumplirá esos requerimientos en un [_Sofware Design Specifications_]{lang=en-US}. Una vez que ambas partes están de acuerdo, se comienza con el desarrollo.

El SRS para FeenoX (@sec-srs) es ficticio pero plausible. Podríamos pensarlo como un llamado a licitación para que alguien desarrolle una herramienta computacional que permita resolver problemas matemáticos con interés práctico de ingeniería. En forma muy resumida, requiere
...
Y que no sólo es de código abierto en el sentido de la OSI sino que es libre en el sentido de la FSF.
La diferencia entre código abierto y software libre es sutil más que práctica, ya que las definiciones técnicas prácticamente coinciden. Pero el punto principal de que el código sea abierto es que permite obtener mejores resultados con mejor performance mientras más personas puedan estudiar el código, escrutarlo y eventualmente mejorarlo. 
...
De hecho, como Unix^[A principios de 1960 llegaron a desarrollar un sistema operativo que funcionaba bien, así que decidieron encarar MULTICS. Como terminó siendo una monstruosidad, empezaron UNIX que es lo que quedó bien.]
y C^[A fines de 1960 llegaron a desarrollar un un lenguaje de programación A que funcionaba bien, así que decidieron encarar B. Como terminó siendo una monstruosidad, empezaron C que es lo que quedó bien.], FeenoX es un "efecto de tercer sistema"^[Del inglés [_third-system effect_]{lang=en-US}] @raymond. Esta diferencia entre el concepto de código abierto y software libre fue discutida en la referencia @enief-milonga-2014 durante el desarrollo de la segunda versión del sistema.

En el SDS definimos la filosofía de diseño del software y elegimos una de las infinitas formas de diseñar una herramienta computacional mencionadas al comienzo del capítulo.
Gran parte de este diseño está basado en la filosofía de programación Unix @raymond.
Damos ejemplos de cómo son las interfaces para definir un cierto problema y cómo obtener los resultados.
Comparamos alternativas e indicamos por qué hemos decidido diseñar el software de la forma en la que fue diseñado.
En este capítulo nos centramos en la implementación, tratando de converger a una de las infinitas formas de implementar el diseño propuesto en el SDS.
...
Por el momento enfoquémonos en problemas lineales como los analizados en los dos capítulos anteriores.
Las dos tareas principales son

 1. construir la matriz global de rigidez $\mat{K}$ y el vector $\vec{b}$ (o la matriz de masa $\mat{M}$)
 2. resolver el sistema de ecuaciones $\mat{K} \cdot \vec{u} = \vec{b}$ (o $\mat{K} \cdot \vec{u} = \lambda \cdot \mat{M} \cdot \vec{u}$)
 
Haciendo énfasis en la filosofía Unix, tenemos que escribir un programa que haga bien una sola cosa^[[Do only one thing but do it well]{lang=en-US}.] que nadie más hace y que interactúe con otros que hacen bien otras cosas.
...
Otra vez desde el punto de vista de la filosofía de programación Unix, la tarea 1 consiste en un [_glue layer_]{lang=en-US} entre la definición del problema a resolver por parte del ingeniero usuario y la biblioteca matemática para resolver problemas ralos^[Del inglés [_sparse_]{lang=en-US}.] PETSc. 
...
Según la regla de representación de Unix, la implementación debería poner la complejidad en las estructuras de datos más que en la lógica. Sin embargo, en el área de mecánica computacional, paradigmas de programación demasiado orientados a objetos impactan en la performance. La tendencia es encontrar una balance entre programación orientada a objetos y programación orientada a datos.
Tal como explican los autores de PETSc, es C el lenguaje que mejor se presta a este paradigma:
...
Fortran fue diseñado en la década de 1950 y fue un salto cualitativo con respecto a la forma de programar las incipientes computadoras digitales. C fue diseñado a principios de la década 1970 suponiendo arquitecturas de hardware que justamente son las que se emplean hoy en día, tales como

 * espacio de direcciones plano^[[Flat address space]{lang=en-US}]
 * registros
 * entrada y salida basada en archivos
 * etc
 
para correr en entornos Unix, que justamente, es el sistema operativo de la vasta mayoría de los servidores de la nube pública.
De la misma manera que el lenguaje Fortran permite realizar ciertas prácticas que bien utilizadas agregan potencia y eficiencia pero cuyo abuso lleva a código ininteligible (por ejemplo el uso y abuso de bloques `COMMON`), C++ permite ciertas prácticas que bien utilizadas agregan potencia pero cuyo abuso lleva a código de baja calidad (por ejemplo el uso y abuso de `template`s).


Hemos decidido entonces construir la matriz $\mat{K}$ y el vector $\vec{b}$ en C utilizando una estructura de datos que PETSc pueda entender.
Preguntémonos ahora qué necesitamos para construir estos objetos.
Para simplificar el argumento, supongamos por ahora que queremos resolver la ecuación generalizada de Poisson de la @sec-poisson. La matriz global $\mat{K}$ proviene de ensamblar las matrices elementales $\mat{K}_i$ de elemento $e_i$ para todos los elementos volumétricos según la @def-Ki-poisson. De la misma manera, el vector global $\vec{b}_i$ proviene de ensamblar las contribuciones elementales $\vec{b}_i$ tanto de los elementos volumétricos (@def-bi-volumetrico-poisson) como de los elementos de superficie con condiciones de contorno naturales (@def-bi-superficial-poisson).
...
\caption{\label{alg:poisson}Posible implementación de alto nivel de la construcción de $\mat{K}$ y $\vec{b}$ para el problema de Poisson generalizado de la @sec-poisson}
\end{algorithm}
```
...
\caption{\label{alg:poisson-dirichlet1}}
\end{algorithm}
```
...
\caption{\label{alg:poisson-dirichlet2}}
\end{algorithm}
```


Una posible implementación en pseudo código de alto nivel de esta construcción podría ser la ilustrada en el algoritmo \ref{alg:poisson}. La aplicación de las condiciones de contorno de Dirichlet según la discusión de la @sec-dirichlet-nh puede ser realizada con el algoritmo \ref{alg:poisson-dirichlet1} o con el algoritmo \ref{alg:poisson-dirichlet2}.
La primera conclusión que podemos extraer es que la ecuación a resolver está determinada por la expresión entre llaves de los términos evaluados en cada punto de Gauss para $\mat{K}_i$ y para $\vec{b}_i$.
A lo sumo deberíamos reemplazar las matrices $\mat{H}_c$ y $\mat{B}_i$ por sus versiones $G$-aware $\mat{H}_{Gc}$ y $\mat{B}_{Gi}$. Por ejemplo, si quisiéramos resolver difusión de neutrones multigrupo tendríamos que hacer
...
\end{aligned}
$$
y tendríamos que agregar contribuciones a la matriz de rigidez para condiciones de Robin.
...
La segunda proviene de preguntarnos qué necesitan estos algoritmos:

 i. los conjuntos de cuadraturas de gauss $\omega_q, \symbf{\xi}_q$ para el elemento $e_i$
 ii. las matrices elementales $\mat{H}_{c}$, $\mat{B}_{c}$, $\mat{C}_i$, $\mat{H}_{Gc}$, $\mat{B}_{Gi}$, etc.
 iii. poder evaluar en $\vec{x}_q$
      ...
      c. las condiciones de contorno $p(\vec{x})$ y $g(\vec{x})$ (o las correspondientes a difusión)

Los dos primeros puntos son genéricos y no dependen de la ecuación a resolver.
Sólo se necesita un analizador de mallas no estructuradas definidas según la posición $\vec{x}_j \in \mathbb{R}^D$ de los nodos y la topología de cada uno de los elementos $e_i$.

El tercer punto en principio sí depende de la ecuación, pero finalmente se reduce a la capacidad de evaluar propiedades de materiales y condiciones de contorno en una ubicación espacial arbitraria $\vec{x}$.
...
Entonces la segunda conclusión es que si nuestra herramienta puede proveer un mecanismo para definir propiedades materiales que puedan depender discontinuamente según el material al que pertenezca cada elemento volumétrico (algunos elementos pertenecerán al combustible y otros al moderador) volumétrico y/o continuamente en el espacio según variaciones locales (por ejemplo cambios de temperatura y/o densidad, concentración de venenos, etc.) y otro para poder definir funciones del espacio asociadas a elementos superficiales para poner condiciones de contorno de forma genérica 
