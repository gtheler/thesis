...
for i in sds why licensing transfer lorenz nafems-le10 mazes laplace nafems-le11 binary source git compilation unix history downloads debian binary source git freesw licensing windows pdes; do
  cp ${dir}/doc/${i}.md .
done
...
sed 's/sds-appendices.md//' sds.md | grep -v "sec-download" > tmp
mv tmp sds.md
